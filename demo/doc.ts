...
};</code></pre><h2 class="block at-heading" id="jW8cAuj0lub" data-toc-id="jW8cAuj0lub">Menu配置</h2><hr class="block at-horizontal-rule"><p class="block at-block">一些菜单项提供了配置属性，具体如下<span style="font-size: 20px; color: rgb(31, 31, 31)">👇</span></p><h4 class="block at-heading" id="at-41d0Nd99rv8" data-toc-id="at-41d0Nd99rv8">1. Heading标题</h4><pre class="block at-code-block"><code class="language-typescript">// 支持配置等级，1-6
&lt;Heading levels={[1, 2, 3, 4, 5, 6]} /&gt;</code></pre><h4 class="block at-heading" id="at-9EHxQGER8Cr" data-toc-id="at-9EHxQGER8Cr">2. LineHeight行高</h4><pre class="block at-code-block"><code class="language-typescript">// 支持配置任意行高
&lt;LineHeight lineHeights={[1, 1.15, 1.3, 1.5, 2, 3]} /&gt;</code></pre><h4 class="block at-heading" id="at-tGOZNGnDKMB" data-toc-id="at-tGOZNGnDKMB">3. FontSize字体大小</h4><pre class="block at-code-block"><code class="language-typescript">// 支持配置任意字体大小，配置项必须包含px
&lt;FontSize fontSize={['14px', '18px', '22px']} /&gt;</code></pre><h4 class="block at-heading" id="at-CWo960Wg1YC" data-toc-id="at-CWo960Wg1YC">4. 图片视频文件</h4><p class="block at-block">Image、Video、File媒体文件配置大小和自定义上传，以Image为例：</p><p class="block at-block">1、使用编辑器内置的上传能力，通过<code>uploadAction</code> 属性配置上传地址，接口响应需返回 <span style="color: rgb(51, 85, 255)"><strong>src</strong> </span>字段</p><pre class="block at-code-block"><code class="language-typescript">&lt;Image
  // 接口响应需返回
...
/&gt;</code></pre><h4 class="block at-heading" id="at-Y3l61kSbfVd" data-toc-id="at-Y3l61kSbfVd">5. 字体颜色与背景色</h4><p class="block at-block">支持配置色板的预设颜色</p><pre class="block at-code-block"><code class="language-typescript">&lt;Color colorPreset={['red', 'blue']} /&gt;</code></pre><pre class="block at-code-block"><code class="language-typescript">&lt;HighlightColor colorPreset={['red', 'blue']} /&gt;</code></pre><h4 class="block at-heading" id="at-ARezTHDBFR8" data-toc-id="at-ARezTHDBFR8">6. Fullscreen全屏</h4><p class="block at-block">激活和取消全屏时触发<span style="color: rgb(51, 85, 255)"><strong>onFullscreenChange</strong></span>事件，同时编辑器的根节点会增加<code>fullscreen</code>的className，开发者通过事件和类名处理全屏显示逻辑</p><pre class="block at-code-block"><code class="language-typescript">&lt;Fullscreen
...
/&gt;</code></pre><h2 class="block at-heading" id="at-c08JhP17eUC" data-toc-id="at-c08JhP17eUC">块级悬浮菜单</h2><p class="block at-block">编辑器支持块级的悬浮菜单，需要自定引入<code>FloatMenu</code> 组件。<a target="_blank" rel="noopener noreferrer nofollow" class="at-link at-link" href="https://code.djicorp.com/fe/antd-richtext/-/blob/master/src/menus/FloatMenu/types.ts">config配置配置项</a></p><pre class="block at-code-block"><code class="language-typescript">import { useRef } from 'react';
...
};</code></pre><h2 class="block at-heading" id="at-VJhuVk7YMk" data-toc-id="at-VJhuVk7YMk">拖拽与粘贴</h2><hr class="block at-horizontal-rule"><p class="block at-block">使用<code>FileHandler</code>扩展可以使编辑器支持将文件拖拽或粘贴到编辑器中，以图片为例</p><pre class="block at-code-block"><code class="language-typescript">// 导入FileHandler扩展
